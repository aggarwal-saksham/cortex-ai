...
CortexAI leverages a clean, decoupled microservices layout:

```mermaid
graph TD
    Client[React Client] -->|HTTP/Cookies| Gateway[API Gateway - Port 8000]
    
    Gateway -->|Proxy auth requests| AuthService[Auth Service - Port 8001]
    Gateway -->|Proxy chat requests| ChatService[Chat Service - Port 8002]
    Gateway -->|Proxy agent requests| AgentService[Agent Service - Port 8003]
    Gateway -->|Proxy billing requests| BillingService[Port 8004]

    AuthService --> MongoDB[(MongoDB)]
    AuthService --> Redis[(Redis Session Store)]
    AuthService --> Firebase[Firebase Auth Admin]

    ChatService --> MongoDB
    
    AgentService --> Redis[(Redis Limit/Memory Store)]
    AgentService --> Qdrant[(Qdrant Vector DB)]
    AgentService --> S3[(AWS S3 Store)]
    AgentService --> LLMs[Groq / Gemini / OpenRouter]
    AgentService --> Tavily[Tavily Search API]

    BillingService --> MongoDB
    BillingService --> Razorpay[Razorpay API Gateway]
```
